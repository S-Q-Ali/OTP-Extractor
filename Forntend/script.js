...
function initializeComponents() {
    ...
    window.forgotPasswordOTPForm = new ForgotPasswordOTPForm(handleForgotPasswordOTP);
...
function initializeEventListeners() {
    ...
    document.getElementById('continue-to-verification').addEventListener('click', function() {
    ...
    document.getElementById('resend-forgot-link').addEventListener('click', function(e) {
        e.preventDefault();
    ...
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Enter') {
            ...
            const submitBtn = activeScreen?.querySelector('button[type="submit"], .primary-btn');
...
async function handleLogin(email, password) {
    ...
    try {
        ...
        window.toast.show('Welcome! Please set up two-factor authentication', 'info');
...
function generateQRCode(email) {
    ...
    setTimeout(() => {
        ...
        window.toast.show('QR code generated! Scan with your authenticator app', 'success');
    }, 1000);
}
