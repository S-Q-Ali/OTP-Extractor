...
const {
  ...
  writeUsers,
  getUserByEmail,
} = require("../utils/userHelpers"); // ✅ fixed
...
async function verifyTotp(req, res) {
  try {
    ...
    const users = await readUsers();
    users.users[email].is_verified = true;
    users.users[email].last_login = new Date().toISOString();
    await writeUsers(users);
...
module.exports = { register, login, verifyTotp };
